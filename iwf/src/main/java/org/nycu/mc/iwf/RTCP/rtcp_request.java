...
extends Rtcp {
    ...
    public rtcp_request(long SSRC_ID, int priority, String user_id, short indicator) {
    	System.out.println("[rtcp_request]"+SSRC_ID+" ,"+priority+" ,"+user_id+" ,"+indicator);
        message_tools tool = new message_tools();
        this.header = this.messageheader("request", SSRC_ID, "MCPT");
        ...
        this.message = new byte[this.header.length + this.Floor_Priority_field.length + this.User_ID_field.length + this.Floor_Indicator_field.length];
    ...
    public void writemessage() {
        ...
        i2 = this.header.length + this.Floor_Priority_field.length;
        j2 = 0;
        while (j2 < this.User_ID_field.length) {
            this.message[i2 + j2] = this.User_ID_field[j2];
            ++j2;
        }
        i2 = this.header.length + this.Floor_Priority_field.length + this.User_ID_field.length;
        j2 = 0;
    ...
    public void get_Floor_Priority_field(byte[] M) {
        if (M[0] != 0) {
            ...
            System.out.println("floor priority = " + this.Priority_int);
        }
    }
    ...
    public void get_Floor_Indicator_field(byte[] M) {
        if (M[0] != 13) {
            ...
            System.out.println("floor indaicator = " + this.Floor_ind);
        }
    }
